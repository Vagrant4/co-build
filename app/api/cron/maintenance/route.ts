...
export async function GET(request: Request) {
  ...
  const [notifications, checkpoint, retention] = await Promise.all([
  ...
  const alertNeeded = !auditVerification.valid || notifications.failed > 0;
  const alert = alertNeeded ? await sendOpsAlert("maintenance_attention_required", { auditValid: auditVerification.valid, failedNotifications: notifications.failed, staleUploadReservations: uploads.count }) : { delivered: false, skipped: true };
  logEvent("info", "maintenance_completed", { requestId, expiredRateLimits: rateLimits.count, staleUploadReservations: uploads.count, deliveredNotifications: notifications.delivered, failedNotifications: notifications.failed, auditCheckpointCreated: checkpoint.created, auditValid: auditVerification.valid });
  return Response.json({ status: auditVerification.valid ? "ok" : "degraded", expiredRateLimits: rateLimits.count, staleUploadReservations: uploads.count, notifications, retention, alert, checkpoint: { created: checkpoint.created, eventCount: checkpoint.eventCount }, auditVerification }, { status: auditVerification.valid ? 200 : 503, headers: { "Cache-Control": "no-store", "x-request-id": requestId } });
}
